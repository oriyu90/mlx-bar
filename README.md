...
Version 2.4.0 — repository: [oriyu90/mlx-bar](https://github.com/oriyu90/mlx-bar)
...
- OMLX相当機能の設定一元管理（v2.4.0）：モデル常駐上限・同時生成ヘッドルーム・モデル別メモリ上限、プロンプトキャッシュ詳細（世代保持・メモリ比率・分岐チェックポイント・書き込み予算）、生成の入力上限・タイムアウト・メモリガード比率、API要求上限、ログレベル、前回モデル復元、LM Studio連携、RAG埋め込み調整、Adaptive Memoryフォールバックを「Settings…」と`mlxbarctl`のnamedコマンドの両方から操作。将来phase用のゲート（RAMホット層・Hybrid KV再利用）は状態表示のみで有効化手段を提供しない
...
1. [GitHub Releases](https://github.com/oriyu90/mlx-bar/releases)から`MLXBar-2.4.0.dmg`をダウンロードして開きます。
...
mlxbarctl config set-context-compression --enabled true --trigger-percent 70 \
  --keep-tail 8 --summary-max-tokens 800          # 指定したオプションだけ変更（v2.0.0〜）
mlxbarctl config set-launch-at-login true        # デスクトップのGUIが次に起動したときにOS登録へ反映されます
mlxbarctl config set-rag --enabled true --embedding-base-url http://127.0.0.1:1234/v1 \
  --embedding-model text-embedding-nomic-embed-text-v1.5 --chunk-size 1000 --default-top-k 4  # v2.1.0〜
mlxbarctl rag status
mlxbarctl rag collection create my-notes
mlxbarctl rag doc add my-notes --file ./notes.md --wait
mlxbarctl rag doc list my-notes
mlxbarctl rag query my-notes "how do I rotate the API key" --top-k 4
mlxbarctl rag collection delete my-notes
mlxbarctl secrets set-rag-embedding-token TOKEN   # 引数省略で削除
...
- `response_format`は`text` / `json_object` / `json_schema`に対応します（v2.0.0〜、プロンプト指示＋生成後バリデーションのベストエフォート。詳細は上を参照）。それ以外のtypeはHTTP 400で拒否します。
- `n`（複数候補生成、1〜8）に対応します（v2.0.0〜、非stream時のみ。`stream: true`との併用はHTTP 400）。
- `/v1/completions`（レガシーの生テキスト補完、chat templateなし）に対応します（v2.0.0〜）。
...
OpenAI Codex CLIは既定でOpenAI **Responses API**（`wire_api = "responses"`）を使います。MLXBarは
Responses APIを実装しておらず、`/v1/responses`には明示的な`404 UNSUPPORTED_ENDPOINT`を返します
（フリーズせず即座にエラー終了します。理由は[`DESIGN_v2.0.0.md`](DESIGN_v2.0.0.md)を参照）。
Codexから使うには、`~/.codex/config.toml`でカスタムプロバイダを定義し、**`wire_api = "chat"`**を
指定してChat Completions経路を使わせます。

```toml
[model_providers.mlxbar]
name = "MLXBar"
base_url = "http://127.0.0.1:11435/v1"
wire_api = "chat"
# APIキー要求が有効な場合は環境変数名を指定し、その変数にトークンを入れる
env_key = "MLXBAR_API_KEY"

[profiles.mlxbar]
model_provider = "mlxbar"
model = "loaded"   # または GET /v1/models が返すモデル名
```

`codex --profile mlxbar`（または `model_provider = "mlxbar"` を `[profiles]` の既定に）で接続します。
`wire_api = "chat"`にすれば、tool calling・ストリーミング・`max_tokens`省略時の既定上限など、
上記「Zed / Cline / OpenCode」と同じ扱いになります。
...
- **extended thinking**（`thinking: {type: "enabled", budget_tokens}`）はv2.0.0から対応します。既存の推論分離機構（`reasoning_delta`）をそのまま`thinking` content blockとして返し、stream/非stream双方で`thinking_delta` / `signature_delta`を含みます。ただし`signature`はMLXBarが自前で計算するローカルな印（`mlxbar-local-unsigned:`接頭辞つき）で、Anthropic本家が発行・検証する暗号署名ではありません。MLXBar自身はどんな`signature`が返ってきても検証しないため、MLXBar同士の往復では問題なく動作しますが、そのまま本家Anthropic APIへ送っても本家側では拒否されます。
...
出力は`dist/MLXBar.app`と`dist/MLXBar-2.4.0.dmg`です。`Packaging/icon.ico`からmacOS用アイコンを生成してアプリへ組み込みます。環境変数`DEVELOPER_ID_APPLICATION`を設定するとその証明書で署名し、未設定時はad-hoc署名します。Apple公証には別途Developer ID資格情報が必要です。
