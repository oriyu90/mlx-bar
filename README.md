...
Version 1.3.1 — repository: [oriyu90/mlx-bar](https://github.com/oriyu90/mlx-bar)
...
1. [GitHub Releases](https://github.com/oriyu90/mlx-bar/releases)から`MLXBar-1.3.1.dmg`をダウンロードして開きます。
...
出力は`dist/MLXBar.app`と`dist/MLXBar-1.3.1.dmg`です。`Packaging/icon.ico`からmacOS用アイコンを生成してアプリへ組み込みます。環境変数`DEVELOPER_ID_APPLICATION`を設定するとその証明書で署名し、未設定時はad-hoc署名します。Apple公証には別途Developer ID資格情報が必要です。
