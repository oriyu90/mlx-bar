...
Version 1.0.0 — repository: [oriyu90/mlx-bar](https://github.com/oriyu90/mlx-bar)
...
- MLX LM / MLX VLMを分離したランタイムの自動更新、検証、復元、旧版削除
...
1. [GitHub Releases](https://github.com/oriyu90/mlx-bar/releases)から`MLXBar-1.0.0.dmg`をダウンロードして開きます。
2. `MLXBar.app`を`Applications`へコピーします。
3. 初回起動時にmacOSの確認が表示された場合は、「システム設定」→「プライバシーとセキュリティ」から起動を許可します。
4. メニューバーのMLXBarアイコンから「ランタイム更新…」を開き、利用する`mlx-lm`または`mlx-vlm`ランタイムをダウンロード・検証して切り替えます。
5. 「モデルを選択…」からモデルをロードします。
...
- 「ランタイム更新…」: 最新版または指定版を別環境へ取得し、検証後に切替・復元・旧版削除
...
「ランタイム更新…」では、`mlx-lm`と`mlx-vlm`を個別に更新できます。「最新版へ自動更新」を押すと、以降は次の処理を自動で行います。
...
出力は`dist/MLXBar.app`と`dist/MLXBar-1.0.0.dmg`です。`Packaging/icon.ico`からmacOS用アイコンを生成してアプリへ組み込みます。環境変数`DEVELOPER_ID_APPLICATION`を設定するとその証明書で署名し、未設定時はad-hoc署名します。Apple公証には別途Developer ID資格情報が必要です。
