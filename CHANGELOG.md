...
v2.4.0実装後の監査で検出した4件を修正するパッチリリースです。新規エンジン機能・
新規設定キーの追加はなく、既定値はすべてv2.4.0と同一です。

### 修正

- `general.logLevel` をpublic APIサーバと管理サーバの両uvicornへ配線
  （起動時latch、listener切替時にも再読込。不正値は`warning`へフォールバック）。
- ログレベルに許容値（debug/info/warning/error）のサーバ検証を追加。
- メモリガード比率と最大同時接続数、ログレベルの適用タイミング注意文を設定画面に追加。
- 1MB未満の要求サイズ表示の丸めを注記。

詳細は[`DESIGN_v2.4.1.md`](DESIGN_v2.4.1.md)、検証記録は[`TEST_PLAN_v2.4.1.md`](TEST_PLAN_v2.4.1.md)を参照してください。
...
- **コンテキスト自動圧縮（既定オフのオプトイン機能）。** ZCode/Claude Code/OpenCodeのような、
  会話全体を毎リクエスト送り続けるクライアントで、長い会話が遅くなる（最終的には
  `INPUT_TOO_LARGE`で失敗する）問題への対処。プロンプトがモデルのコンテキスト窓に対する
  割合（既定70%）を超えると、会話の中間部分（先頭のsystem/tool定義・直近の会話・
  `tool_calls`/`tool`のペア・画像を含むターンは常に除外）を、既にロード済みの同じモデルで
  1件の要約メッセージへ置き換えます。要約生成が失敗した場合は元のプロンプトのまま送られます。
  **既定は無効**（`contextCompression.enabled: false`）。Settingsの「コンテキスト自動圧縮」
  セクションで有効化・閾値・保持ターン数・要約の最大トークン数を設定できます。
- **メニューバーのモデル一覧UIを刷新。** 「主モデル1件を大きく表示＋その下に常駐モデルの内訳」
  という重複表示を廃止し、全常駐モデル（Quick Chatの対象は「既定」バッジ表示）を対等な1リストへ
  統一。各行の左側にコピーボタンを追加（主モデルに限らずどのモデル名もコピー可能）、
  ロード中／応答生成中／順番待ち／待機中を常に示すステータス行を新設し、生成中はtok/sを
  併記します。バックエンドは無変更（既存の`laneQueueDepth`等をSwift側で利用）。

### テスト

- Pythonテスト**388件**（v1.9.1の380＋新規8）: 既定無効時は無変更のまま送られること、
  `tool_calls`/`tool`ペアを分断しないこと、要約失敗時に元のプロンプトへフォールバックすること、
  画像を含むターンが圧縮候補に入らないこと、短い会話では発火しないこと、設定のバリデーション。
  連続3回実行して安定。
...
- **v1.6.0のスナップショット再利用が、注意層と再帰層が混在するモデルすべてで一度も有効になっていなかった問題を修正しました。** 能力判定はロード直後の空のキャッシュに「状態を取り出せるか」を尋ねますが、空の`KVCache`は`state`ゲッターが`self.keys.shape`を読むため`AttributeError`になり、`hasattr`はそれを「属性なし」として報告します。判定がインスタンスの値ではなく型のプロパティを見るようにしました。Qwen3.5/3.8系のように再帰層を含むモデルは、これまで`rollbackCapability: none`のまま枝分かれのたびにプロンプト全体を再計算していました。
- **巻き戻しの事前判定をすり抜けて`'ArraysCache' object has no attribute 'trim'`に到達する経路を塞ぎました。** ランタイムは破棄するトークン数をキャッシュ自身のoffsetから計算するため、キャッシュが自分のラベルより先行していると、単なる継続でも`trim()`が呼ばれます。返す共通prefix長が破棄を生まないことまで確認してから答えるようにしました。
- **スナップショットの差し替えを、保持しているキャッシュの中身の入れ替えに変更しました。** mlx-vlm 0.6.15は`.cache`をnull検査で1回、prefix長を尋ねた後にもう1回読みます。v1.6.0の順序ガードは前者を数えてしまい、復元経路に一度も入れませんでした。呼び出し順に依存しない形にしたので、将来ランタイムが先に読む実装になっても、返す長さと使われるキャッシュが食い違いません。この経路が`.cache`を`None`にしなくなったのも同じ理由です（ランタイムは自分のnull検査を通過済みで、その後に破棄量を計算します）。
- **スナップショットのディスク書き込みが、毎回失敗して数百MBの残骸だけを残していた問題を修正しました。** `mx.save_safetensors`は拡張子で終わらないパスに`.safetensors`を足すため、`<digest>.safetensors.tmp`へ書くと、後続のchmod・rename・容量管理がどれも見つけられない第三の名前ができていました。一時ファイル名を拡張子で終わるようにし、未完了の書き込みは次のロード時に回収します。この経路はv1.6.0では到達しなかったため表面化していません。
- **キャッシュ世代の掃除が、チェックポイント保存先ごと削除しうる状態でした。** APCの世代とチェックポイントの保存先は同じ親ディレクトリに並ぶため、掃除の対象をAPC自身の世代だけに限定しました。
- **cold理由と共通prefix長がアクセスログへ渡っていなかった問題を修正しました。** 列・保存処理・Worker・設定画面はv1.6.0から揃っていましたが、それらを運ぶ1か所だけが記録対象に含めておらず、`cold_reason`は常にNULLでした。再試行で回復した要求も、判定に使ったprobeが差し替えで失われないよう引き継ぎます。
- **アプリを更新した直後にAPIサーバーが起動できなくなることがある問題を修正しました。** 終了→差し替え→起動という手順では、前のプロセスが待ち受けていたポートがまだTIME_WAITに残ります。`SO_REUSEADDR`を設定して、この状態だけを許容します（別のプロセスが実際に待ち受けているポートは従来どおり失敗し、競合として表示されます）。
- **キャッシュ状況の取得が、生成中は10秒で切れて素の500になっていた問題を修正しました。** WorkerはRPCをモデルと同じスレッドで処理するため、27B級の生成中は数分間答えられません。「再利用は効いているか」は遅い要求のときにこそ聞かれるので、分かっている値を`stale`と明示して返します。
- **管理APIの想定外エラーが、どこにも記録されないまま`Internal Server Error`だけ返していた問題を修正しました。** 経路とtracebackをサービスログへ残し、本文でも種別を返します。
- **存在しないモデル名が、他の要求の実行中は`ENGINE_BUSY`（429、再試行可）になっていた問題を修正しました。** モデルの解決を混雑判定より先に行い、HTTP 404の`MODEL_NOT_FOUND`を返します。

### 追加

- **`usage.prompt_tokens_details.cached_tokens`を返します。** OpenAI互換クライアントが再利用量を読む標準の場所です。ランタイムが報告しなかった場合は0を書かずフィールドごと省きます。
- **`GET /v1/models`が`modalities`を返し、生成に使えないカタログ行を一覧から外します。** 拡散モデルの`vae`・`text_encoder`・`transformer`のような、外形がMLXの重みと区別できないフォルダを、モデル探索するクライアントへ提示しなくなります。管理API側は理由を添えて従来どおり全件返します。
- **READMEにOpenClaw向けの設定手順を追加しました。** OpenClawの無応答監視はSSEコメントを数えないため、`models.providers.<id>.timeoutSeconds`の指定が必要です。

### 性能

- ストリーム中のusage見積りが、tokenごとにmessages全体をJSON化していました。プロンプト長はストリーム中に変わらないため1回だけ測るようにしています（実測: 33KBのmessagesで0.088 ms/token、400KB相当で1.1 ms/token）。

### 検証

- Pythonテスト274件が成功（能力判定・順序非依存の復元・破棄量の不変条件・スナップショットの書き込み名・世代掃除の範囲・usageの再利用報告・カタログ提示を対象とする20件を追加）。
- Swiftリリースビルドが成功。
- 実機計測と、重みなしで実ランタイムに対して走らせる`scripts/verify-prompt-cache-runtime.py`の結果は`TEST_PLAN_v1.6.1.md`に記載。
