...
### 公開APIの認証前リソース消費（未対応・要修正）

**症状**: 認証情報なしのリクエストでも、本文が最後まで読まれてJSON解析されてから401が返る。ボディサイズ上限・接続数上限・レート制限がいずれも無い。

**実測（2026-08-22、隔離インスタンス）**:
- 認証なしで76 MBのJSONを送信 → `uploaded=80,000,062B`、RSS 373 MB → 455 MB、その後401。
- 同じものを8並列 → RSS 602 MB → **1,167 MB**。リクエスト完了後も戻らない。

**原因**: FastAPIのハンドラ引数`body: dict`が、ハンドラ本体の`authorize(request)`より前に本文全体をパースする。壊れたJSONが401ではなく422を返すことでも確認できる（`POST /v1/chat/completions`に無効なJSONを認証なしで送ると422）。

**なぜ問題か**: 27Bをユニファイドメモリへ載せた機体で、認証情報を持たない相手にメモリを取られる。LAN公開だけでも成立し、インターネット公開時はポートスキャンやbot巡回だけで起きる。

**修正方針**:
1. 認証を本文パースより前へ出す。ハンドラ引数での受け取りをやめ、`authorize(request)`後に`await request.json()`する。あるいはASGIミドルウェア／`APIRouter(dependencies=[Depends(...)])`で先に認証する。
2. `Content-Length`に対するボディサイズ上限を入れ、超過は本文を読まずに413で切る。上限は`generation.maxPromptCharacters`と画像上限から算出できる。
3. uvicornの`limit_concurrency`と、認証失敗のレート制限（同一IPの連続失敗にバックオフ）を検討する。
4. `/health`は現状も認証不要。公開面のフィンガープリントになるので、LAN公開時だけ絞るか現状維持かを決める。

**注意**: これはHTTP公開の可否とは独立の欠陥。VPN経由で使う場合でも直す価値がある。
